...
var (
	errInvalidName    = errors.New("must not use _ prefix/suffix")
	errReservedWords  = errors.New("reserved word, should not be used")
...
func isColumnNameValid(name string) error {
	return isNameReserved(name)
}

func isNameReserved(name string) error {
	name = strings.ToLower(name)
	// error out on _ prefix/suffix
	if strings.HasPrefix(name, "_") || strings.HasSuffix(name, "_") {
		return errInvalidName
	}
	// check name against reserved words
	for _, v := range reserved {
		if name == strings.ToLower(v) {
			return errReservedWords
		}
	}
	return nil
}
